...
export const FilteredMoviesSkeleton = () => {
  return (
    <Box sx={{display: "grid",gridTemplateColumns: {xs: "repeat(2, 1fr)", sm: "repeat(3, 1fr)",md: "repeat(4, 1fr)"}, gap: 3}}>
      {Array.from({ length: 8 }).map((_, index) => (
        <Box key={index}>
          <Skeleton variant="rectangular" animation="wave" sx={{width: 200,  height: 300, borderRadius: 2, backgroundColor: "rgba(87, 86, 86, 0.972)"}}/>
